...
将 .dsh 目录复制到 $DSH_HOME/.agent-presets/hermes-dsh-integration/：

    # Windows PowerShell 7
    $dst = Join-Path $env:DSH_HOME ".agent-presets\hermes-dsh-integration"
    Copy-Item -Recurse -Force ".\dsh" $dst

重启 DeepSeek Harness 后，在会话预设列表选择 Hermes DSH 总集成。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 $DSH_HOME/skills/dsh-integration/：

    $dst = Join-Path $env:DSH_HOME "skills\dsh-integration"
    New-Item -ItemType Directory -Force -Path $dst | Out-Null
    Copy-Item ".\skills\dsh-integration\SKILL.md" $dst -Force
